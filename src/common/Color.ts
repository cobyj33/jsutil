...
export class Color implements RGBA {
    ...
    static fromCSS(name: string): Color {
        if (isCSSColorName(name)) {
            const color: RGB | RGBA = getColorDataFromCSS(name)
...
export const CSS_COLOR_NAMES_TO_HEX = Object.freeze({
    "aliceblue": "#f0f8ff",
    "antiquewhite": "#faebd7",
    "aqua": "#00ffff",
    "aquamarine": "#7fffd4",
...
const CSS_HEX_VALUES_TO_CSS_NAME = flipObject(CSS_COLOR_NAMES_TO_HEX)


export function isCSSColorName(str: string): str is CSSColorName {
    return str in CSS_COLOR_NAMES_TO_HEX
}


export function isCSSHexColorValue(str: string): str is CSSColorName {
    return str in CSS_HEX_VALUES_TO_CSS_NAME
}

export function getHexFromCSSColorName(color: CSSColorName): string {
...
const VALID_HEX_DIGIT_STRING_LENGTHS = [SHORTHAND_HEXIDECIMAL_COLOR_STRING_LENGTH, SHORTHAND_WITH_ALPHA_HEXIDECIMAL_COLOR_STRING_LENGTH, HEXIDECIMAL_COLOR_STRING_LENGTH, HEXIDECIMAL_WITH_ALPHA_COLOR_STRING_LENGTH] as const
const VALID_SHORTHAND_HEX_DIGIT_STRING_LENGTHS = [SHORTHAND_HEXIDECIMAL_COLOR_STRING_LENGTH, SHORTHAND_WITH_ALPHA_HEXIDECIMAL_COLOR_STRING_LENGTH] as const
const VALID_ALPHA_HEX_DIGIT_STRING_LENGTHS = [SHORTHAND_WITH_ALPHA_HEXIDECIMAL_COLOR_STRING_LENGTH, HEXIDECIMAL_WITH_ALPHA_COLOR_STRING_LENGTH] as const
...
export function isValidHexColorString(str: string): boolean { 
    if (str.startsWith("#")) {
        if (VALID_HEX_DIGIT_STRING_LENGTHS.some(len => str.length === len)) {
            return isRawHexNumberString(str.slice(1))
        }
    }
    return false;
}
...
**/
export function isValidShorthandHexColorString(str: string): boolean {
    if (str.startsWith("#")) {
...
export function isHexColorStringWithAlpha(str: string): boolean {
 ...
 **/
export function expandShorthandHexColorString(str: string): string {
 ...
 */
export function parseHexColorString(str: string): RGB | RGBA {
    if (isValidHexColorString(str)) {
        ...
        if (isHexColorStringWithAlpha(expandedHexString)) {
            return {
                red: parseRawHexNumberString(expandedHexString.slice(1, 3)),
                green: parseRawHexNumberString(expandedHexString.slice(3, 5)),
                blue: parseRawHexNumberString(expandedHexString.slice(5, 7)),
                alpha: parseRawHexNumberString(expandedHexString.slice(7, 9))
            }
        }
...
export function getColorDataFromCSS(str: CSSColorName): RGB | RGBA {
    const hexStr = getHexFromCSSColorName(str)
    return parseHexColorString(hexStr)
}

export function isRGBObject(obj: any): obj is RGB {
...
export function rgbaToCSSString(color: RGBA): string {
    return `rgba(${color.red}, ${color.green}, ${color.blue}, ${color.alpha})`;
}

export function areEqualColors(first: RGBA | RGB, second: RGBA | RGB): boolean {
